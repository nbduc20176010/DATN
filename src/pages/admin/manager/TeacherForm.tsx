...
export default function TeacherForm({ form }: any) {
  ...
  const { teacherFormOpen, formType } = useAppSelector((state) => state.common);
  ...
  return (
    ...
    >
      ...
      >
        {formType === "add" ? (
          <>
            <Form.Item
              name="username"
              label="Username"
              rules={[{ required: true, message: "Field required!", min: 6 }]}
            >
        ...
        <div className="m-2">
          <input multiple={false} type="file" onChange={uploadingImage} />
        </div>
        <Form.Item
          name="fullName"
          label="Full name"
